...
public final class ExtDataUtil {
	 ...
	 * @return the ext data
	 */
	public static <T> ExtData<T> genWithInfo(String info) {
		return getExtData(0, null, 0, true, info);
	}
	 ...
	 * @return the ext data
	 */
	// private static <T> ExtData<T> getExtData(int status, List<T> root, int
	// totalproperty,
	// boolean success, String syserr) {
	// ExtData<T> data = new ExtData<T>();
	// data.setRows(root);
	// data.setSuccess(success);
	// data.setRespinfo(syserr);
	// data.setTotal(totalproperty);
	// data.setAfterAction("0");
	// return data;
	// }

	private static <T> ExtData<T> getExtData(int status, List<T> root,
			int totalproperty, boolean success, String syserr) {
		ExtData<T> data = new ExtData<T>();
		data.setRows(root);
		data.setResp_code(String.valueOf(success));
		data.setResp_info(syserr);
		data.setTotal(String.valueOf(totalproperty));
		return data;
	}
	 ...
	 * @return the ext data
	 */
	public static <T> ExtData<T> genWithEmpty() {
		return getExtData(0, null, 0, true, "");
	}
