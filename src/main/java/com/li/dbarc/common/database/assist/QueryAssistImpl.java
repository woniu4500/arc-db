package com.li.dbarc.common.database.assist;
...
import com.li.dbarc.common.SystemErrorCode;
import com.li.dbarc.common.database.BaseDao;
import com.li.dbarc.common.database.ExtData;
import com.li.dbarc.common.database.Filter;
import com.li.dbarc.common.database.Page;
import com.li.dbarc.common.database.Pagination;
import com.li.dbarc.common.database.PaginationData;
import com.li.dbarc.common.database.Sort;
import com.li.dbarc.common.database.assist.handler.BaseStatementHandler;
import com.li.dbarc.common.database.assist.handler.ColumnMapHandler;
import com.li.dbarc.common.database.assist.handler.CountStatementHandler;
import com.li.dbarc.common.database.assist.handler.HandlerCollection;
import com.li.dbarc.common.database.assist.handler.QueryType;
import com.li.dbarc.common.database.criteria.BeanColumnMap;
import com.li.dbarc.common.database.criteria.CriteriaUtil;
import com.li.dbarc.common.database.exception.BaseException;
import com.li.dbarc.common.database.exception.UpdateException;
import com.li.dbarc.common.database.support.SqlSessionDaoSupport;
import com.li.dbarc.common.database.util.PageUtil;

/**
 * The Class QueryAssistImpl. 保证此对象为单例运行
 * 
 * @author
...
public class QueryAssistImpl extends SqlSessionDaoSupport implements
		QueryAssist {
	...
	/**
	 * Initialize Query Assist 1. Find out select statement 2. Add _list,
	 * _count, _page statement 3. Map query columns
	 *
	 * @throws Exception
	 *             the exception
	 */
	...
	public void initAssist() throws Exception {
		...
		Set<String> excludeSet = this.excludeStatement == null ? new HashSet<String>()
				: new HashSet<String>(this.excludeStatement);
		...
		handler.addHandlers(new CountStatementHandler(config, QueryType.COUNT),
				new BaseStatementHandler(config, QueryType.PAGE),
				new BaseStatementHandler(config, QueryType.LIST), colMapHandler);
		//
		for (String nm : msNmC) {
	...
	public Long count(String mappedId, List<Filter> filters, Map params) {
		...
		if (this.beanColumnMap.containsKey(mappedId)) {
			...
			parameter.put(F_CRITERIA,
					CriteriaUtil.genCriteriabyFilter(filters, columnMap));
		}
		return (Long) getSqlSession().selectOne(
				QueryType.COUNT.statementId(mappedId), parameter);
	}
	 ...
	 * @param filters
	 * @return
	 * @see com.li.dbarc.common.database.assist.QueryAssist#list(java.lang.String,
	 *      java.util.List)
	 */
	 ...
	 * @return
	 * @see com.li.dbarc.common.database.assist.QueryAssist#list(java.lang.String,
	 *      java.util.List, com.li.dbarc.model.common.Page)
	 */
	 ...
	 * @return
	 * @see com.li.dbarc.common.database.assist.QueryAssist#list(java.lang.String,
	 *      java.util.List, com.li.dbarc.model.common.Page, java.util.Map)
	 */
	public <E> List<E> list(String mappedId, List<Filter> filters, Page page,
			Map params) {
		Map parameter = params == null ? new HashMap() : new HashMap(params);
		...
		return getSqlSession().selectList(QueryType.LIST.statementId(mappedId),
				parameter);
	}
	 ...
	 * @param handler
	 */
	public void selectWithHandler(String statementId, Map parameter,
	 ...
	 * @param mapKey
	 * @return
	 * @see com.li.dbarc.common.database.assist.QueryAssist#map(java.lang.String,
	 *      java.util.List, com.li.dbarc.model.common.Page, java.util.Map,
	 *      java.lang.String)
	 */
	public <K, V> Map<K, V> map(String mappedId, List<Filter> filters,
			Page page, Map params, String mapKey) {
		Map parameter = params == null ? new HashMap() : new HashMap(params);
		if (this.beanColumnMap.containsKey(mappedId)) {
			Map columnMap = this.beanColumnMap.get(mappedId);
			parameter.put(F_CRITERIA,
		...
		return getSqlSession().selectMap(QueryType.LIST.statementId(mappedId),
				parameter, mapKey);
	}
	 ...
	 * @param page
	 * @return
	 * @see com.li.dbarc.common.database.assist.QueryAssist#page(java.lang.String,
	 *      java.util.List, com.li.dbarc.model.common.Page)
	 */
	 ...
	 *      java.util.List, com.li.dbarc.model.common.Page, java.util.Map)
	 */
	public <E> ExtData<E> page(String mappedId, List<Filter> filters,
			Page page, Map params) {
		...
		data.setResp_code("true");
		// data.setSuccess(true);
		// When limit == -1 return all data without pagination .
		// Mainly used in the data export module.
		if (pagination.getLimit() == -1) {
			List<E> result = getSqlSession().selectList(
					QueryType.LIST.statementId(mappedId), parameter);
			data.setRows(result);
			// data.setTotal(result.size());
			data.setTotal(String.valueOf(result.size()));
			return data;
		}
		// Get the total count
		List cnt = getSqlSession().selectList(
				QueryType.COUNT.statementId(mappedId), parameter);
		...
		// data.setTotal(totalCnt);
		data.setTotal(String.valueOf(totalCnt));
		...
		List<E> result = getSqlSession().selectList(
				QueryType.PAGE.statementId(mappedId), parameter);
		data.setRows(result);
		return data;
	}
	...
	public <T> T merge(BaseDao<T> dao, T obj) throws BaseException {
		...
		if (r == 0) {
			throw new UpdateException(SystemErrorCode.CODE_9902,
					SystemErrorCode.CODE_9902_MSG);
		}
	...
	public <T> T insertFetch(BaseDao<T> dao, T obj) throws BaseException {
		...
		logger.error("Dao {} or Object {} is invalid. ", new Object[] { dao,
				obj });
		return obj;
	}
	...
	public <T> T updateFetch(BaseDao<T> dao, T obj) throws BaseException {
		if (dao != null && obj != null) {
			...
			if (r == 0) {
				throw new UpdateException(SystemErrorCode.CODE_9902,
						SystemErrorCode.CODE_9902_MSG_1);
			}
			return dao.findByKey(obj);
		}
		logger.error("Dao {} or Object {} is invalid. ", new Object[] { dao,
	...
	public <T> int insertBatch(BaseDao<T> dao, Collection<T> list) {
		if (dao == null || list == null) {
			logger.error("Dao {} or Collection {} is invalid. ", new Object[] {
					dao, list });
			return 0;
		}
	 ...
	 * @param mappedId
	 * @return
	 * @see com.li.dbarc.common.database.assist.QueryAssist#columnMap(java.lang.String)
	 */
	 ...
	 * 设置 忽略处理的语句名称.
	 *
	 * @param excludeStatement
	 *            the excludeStatement to set
	 */
	...
	/**
	 * @param extendSqlSessionFactory
	 *            the extendSqlSessionFactory to set
	 */
//	public void setExtendSqlSessionFactory(
//			SqlSessionFactory extendSqlSessionFactory) {
//		this.extendSqlSessionFactory = extendSqlSessionFactory;
//	}

//    @Autowired  
//    public void setMySessionFactory(SqlSessionFactory sqlSessionFactory){  
//        super.setSqlSessionFactory(sqlSessionFactory);  
//    }  
	
	/**
	 * @param allowBatchMode
	 *            the allowBatchMode to set
	 */
	 ...
	 * @throws Exception
	 */
	public <T> List<T> queryForListPage(String sqlName, String countSql,
			PaginationData pageList) throws BaseException {

		if (StringUtils.isNotEmpty(sqlName) && null != pageList) {

			/** 查询数据总条数,默认为无数据 */
			String countStr = "0";

			countStr = getSqlSession().selectOne(countSql, pageList.getParam());
			...
			if (StringUtils.isNotEmpty(countStr)) {
				...
				String startIndex = String.valueOf((pageList.getCurPage() - 1)
						* pageList.getPageSize());

				String endIndex = String.valueOf(pageList.getCurPage()
						* pageList.getPageSize());

				if (StringUtils.isNotBlank(startIndex)
						&& StringUtils.isNotEmpty(endIndex)) {
			...
			throw new BaseException("empty Statment Or Param");
